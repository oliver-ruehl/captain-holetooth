extends Node2D

@onready var tabs = get_node("settings")


func _ready():
	var debug = find_child("Debug")
	if !Global.debug_mode and debug:
		debug.get_parent().remove_child(debug)
	

func _input(event):
	if event.is_action_pressed("ui_left"):
		if tabs.current_tab-1<0:
			tabs.current_tab = tabs.get_tab_count()-1
		else:
			tabs.current_tab = tabs.current_tab-1
	elif event.is_action_pressed("ui_right"):
		if tabs.current_tab+1>=tabs.get_tab_count():
			tabs.current_tab = 0
		else:
			tabs.current_tab = tabs.current_tab+1
			
	if event.is_action_pressed("ui_cancel"):
		get_parent()._on_exitbutton_pressed()

#Workaround bug moving right from instructions makes wrong focus on buttons
func _on_settings_tab_changed( tab ):
	if tabs.get_current_tab_control().get_children().size()>0 && tabs.get_current_tab_control().name == "Debug":
		tabs.get_current_tab_control().set_process(true)
